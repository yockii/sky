...
function getOriginalPath(record: { aliasOf: { path: any }; path: any }) {
  return record
    ? (
      record.aliasOf
        ? record.aliasOf.path
...
function isSameRouteRecord(a: { aliasOf: any }, b: { aliasOf: any }) {
...
function includesParams(outer: { [x: string]: any }, inner: { [x: string]: any }) {
  for (const key in inner) {
    ...
    if (typeof innerValue === 'string') {
      ...
      || innerValue.some((value: any, i: string | number) => value !== outerValue[i])
    ) {
      return false
    }
  }
...
export const routerLinkProps = {
  ...
  activeClass: {
    type: String,
    default: 's-router-link--active'
  },
  exactActiveClass: {
    type: String,
    default: 's-router-link--exact-active'
  },
  disable: Boolean
}
...
export default function () {
  ...
  function navigateToLink(e: { metaKey: any; altKey: any; ctrlKey: any; shiftKey: any; __sNavigate: boolean; defaultPrevented: boolean; button: number }) {
    if (
      ...
      || (e.__sNavigate !== true && e.defaultPrevented === true)
