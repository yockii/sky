...
process.on('exit', code => {
    if (code === 0 && tableData.length > 0) {
        ...
        console.log()
        console.log(` Summary of SkyUI v${version}:`)
        console.log(output)
    }
})
...
function getDestinationInfo(dest) {
    ...
    if (dest.endsWith('.js')) {
        return {
            ...
            toTable: dest.indexOf('dist/skyui') > -1
        }
    }

    if (dest.endsWith('.css') || dest.endsWith('.scss')) {
        return {
            banner: blue('[css] '),
...
module.exports.rollupSkyUIUMD = function (config = {}) {
    return {
        name: 'skyui-umd',
        transform(code) {
            return {
                code: `Skyui.${config.type}.set(${code.replace('export default ', '')})`
            }
        }
    }
}
