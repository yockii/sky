...
function writeSkyuiPluginProps(contents, nameName, props, isLast) {
    writeLine(contents, `${nameName}: {`, 1)
    props.forEach(prop => writeLines(contents, prop, 2))
    writeLine(contents, `}${isLast ? '' : ','}`, 1)
}

function addSkyuiPluginOptions(contents, components, directives, plugins) {
    writeLine(contents, 'import { GlobalSkyuiLanguage, GlobalSkyuiIconSet } from \'./globals\'')
    writeLine(contents, 'export interface SkyuiPluginOptions {')
    writeLine(contents, 'lang: GlobalSkyuiLanguage,', 1)
    writeLine(contents, 'config: any,', 1)
    writeLine(contents, 'iconSet: GlobalSkyuiIconSet,', 1)
    writeSkyuiPluginProps(contents, 'components', components)
    writeSkyuiPluginProps(contents, 'directives', directives)
    writeSkyuiPluginProps(contents, 'plugins', plugins, true)
    writeLine(contents, '}')
    writeLine(contents)
}

function addSkyuiLangCodes(contents) {
    ...
    writeLine(contents, 'export interface SkyuiLanguageCodesHolder {', 2)
...
function writeIndexDTS(apis) {
    ...
    writeLine(contents)
    writeLine(skyuiTypeContents, 'export as namespace skyui')
    // We expose `ts-helpers` because they are needed by `@skyui/app` augmentations
    writeLine(skyuiTypeContents, 'export * from \'./ts-helpers\'')
    writeLine(skyuiTypeContents, 'export * from \'./utils\'')
    writeLine(skyuiTypeContents, 'export * from \'./composables\'')
    writeLine(skyuiTypeContents, 'export * from \'./feature-flag\'')
    writeLine(skyuiTypeContents, 'export * from \'./globals\'')
    writeLine(skyuiTypeContents, 'export * from \'./extras\'')
    writeLine(skyuiTypeContents, 'export * from \'./lang\'')
    writeLine(skyuiTypeContents, 'export * from \'./api\'')
    ...
    apis.forEach(data => {
        ...
        // Declare class
        writeLine(skyuiTypeContents, `export const ${typeName}: ${extendsVue ? `ComponentConstructor<${typeName}>` : typeName}`)
    ...
    Object.keys(extraInterfaces).forEach(name => {
        if (extraInterfaces[name] === void 0) {
            // If we find the symbol as part of the generated Skyui API,
    ...
    addSkyuiPluginOptions(contents, components, directives, plugins)

    skyuiTypeContents.forEach(line => write(contents, line))

    writeLine(contents, 'export const Skyui: { install: (app: App, options: Partial<SkyuiPluginOptions>) => any } & QSingletonGlobals')
    writeLine(contents, 'export default Skyui')
    writeLine(contents)
